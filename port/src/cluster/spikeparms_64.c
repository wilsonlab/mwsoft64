...
typedef struct trange_type {
  unsigned int32_t	tstart;
  unsigned int32_t	tend;
...
int16_t intcompare(i1,i2)
     int16_t	*i1,*i2;
{
  return(*i1 - *i2);
}

int16_t CheckTimestampRange(timestamp,result)
     unsigned int32_t	timestamp;
     Result		*result;
{
  TRange	*trange;
...
*********************************************************
*/
int16_t BinaryDualGetSpike(result,data1,data2,fp,timestamp)
Result		*result;
int16_t		*data1;
int16_t		*data2;
FILE	*fp;
...
int16_t		i;
    ...
    ** read in the data
    */
    if(fread(timestamp,sizeof(unsigned int32_t),1,fp) != 1){
	return(0);
    }
...
int16_t BinaryTetGetSpike(result,data1,data2,data3,data4,fp,timestamp)
Result		*result;
int16_t		*data1;
int16_t		*data2;
int16_t		*data3;
...
int16_t DualGetSpike(data1,data2,fp)
int16_t		*data1;
int16_t		*data2;
FILE	*fp;
{
...
int16_t TetGetSpike(data1,data2,data3,data4,fp)
int16_t		*data1;
int16_t		*data2;
int16_t		*data3;
int16_t		*data4;
...
unsigned int32_t	uval;
	...
	*/
	for(i=0;i<nparms;i++){
		...
		fwrite(&uval,sizeof(unsigned int32_t),1,fp);
		break;
	    case DOUBLE:
		...
		fwrite(&ival,sizeof(int16_t),1,fp);
		break;
	    case CHAR:
...
int16_t Offset(max,tpeak,offset)
int16_t	max;
int16_t	tpeak;
int16_t	offset;
{
     ...
     Index		*idx;
{
  int16_t	first;
  int16_t	count;
  int16_t	processed_count;
  unsigned int32_t	 timestamp;
  unsigned int32_t	 etimestamp;
  unsigned int32_t	 stimestamp;
  int16_t		processed;
  int16_t		i;
  int16_t		bin;
  OldPosRecord	oldpos;
  ...
  int16_t		oldx,oldy;
  int16_t           posx, posy;
  unsigned int32_t oldtimestamp;
  double	*vqueue;
  double	*pxqueue;
  double	*pyqueue;
  int16_t	current,prev;
  double	avgvel;
  int16_t	queuesize;
  double	recentvel;
  double	recentx;
  ...
  */
  ...
  while(!feof(xchannel->fp)){
	...
	if(position_file){
		...
		if (backdiode) {
		  posx = (int16_t) oldpos.x2;
		  posy = (int16_t) oldpos.y2;
		} else {
		  posx = (int16_t) oldpos.x1;
		  posy = (int16_t) oldpos.y1;
		}
...
void AddRange(Result *result,unsigned int32_t tstart,unsigned int32_t tend)
/* Result	*result; */
/* unsigned int32_t tstart; */
/* unsigned int32_t tend; */
{
...
char	**header;
    ...
    */
	...
	if(binaryformat){
	    ...
	    */
	    if(fread(&tstart,sizeof(unsigned int32_t),1,fp) != 1){
		break;
	    }
	    /*
	    ** read the ending timestamp
	    */
	    if(fread(&tend,sizeof(unsigned int32_t),1,fp) != 1){
		break;
	    }
	} else {
...
Index	index;
  ...
  spikelen = 0;
  showparms = 0;
  index.index = (int16_t *)malloc(MAX_VECTORS*sizeof(int16_t));
  index.n = 0;
  fpindex = NULL;
  xchannel.fp = NULL;
  ychannel.fp = NULL;
...
/* passed in from makefile when building an RPM*/
  ...
  if(index.n == 0){
	...
	exit(0);
       ...
       ** sort the indices
       */
      qsort(index.index,index.n,sizeof(int16_t),intcompare);
    }
  }
  /*
   ...
   */
  if(result.outputformat == BINARY)
    fprintf(result.fpout,"%% Fields:\tid,%d,%d,%d\t",INT,sizeof(int16_t),1);
  else /* ASCII */
  ...
  if(result.outputformat == BINARY){
    ...
    fwrite(&ival,sizeof(int16_t),1,result.fpout);
  ...
  xchannel.data = (int16_t *)malloc(spikelen*sizeof(int16_t));
  ychannel.data = (int16_t *)malloc(spikelen*sizeof(int16_t));
  achannel.data = (int16_t *)malloc(spikelen*sizeof(int16_t));
  bchannel.data = (int16_t *)malloc(spikelen*sizeof(int16_t));
  ...
  if(analyze){
	...
	sparms[i].hist = (int16_t *)calloc(sparms[i].nbins,sizeof(int16_t));
      }
    }
    /*
     ** rewind the files
