...
*/
    ...
    */
    ...
    (uint32_t *)calloc(nspikes,sizeof(uint32_t));
    /*
    ** allocate the time bin array to be used to compute
    ** bin variance
    */
    binarray = (int32_t *)calloc(result->ntimebins,sizeof(int32_t));
    /*
...
int32_t	convert;
    ...
    */
    if(fread(&xmax,sizeof(int32_t),1,fp) != 1){
	fprintf(stderr,
	"ERROR: unable to read position file for cluster %d\n", clusterid);
	return(NULL);
    }
    if(convert){
	ConvertData(&xmax,sizeof(int32_t));
    }
    if(fread(&ymax,sizeof(int32_t),1,fp) != 1){
	fprintf(stderr,
	"ERROR: unable to read position file for cluster %d\n", clusterid);
	return(NULL);
    }
    if(convert){
	ConvertData(&ymax,sizeof(int32_t));
    }
    ...
    if(fread(&datatype,sizeof(int32_t),1,fp) != 1){
	fprintf(stderr,
	"ERROR: unable to read position file for cluster %d\n", clusterid);
	return(NULL);
    }
    if(convert){
	ConvertData(&datatype,sizeof(int32_t));
    }
    if(datatype != FLOAT){
	fprintf(stderr,
...
int32_t	datatype;
	...
	*/
	fwrite(&(result->xmax),sizeof(int32_t),1,fp);
	fwrite(&(result->ymax),sizeof(int32_t),1,fp);
	fval = 1.0;
	fwrite(&fval,sizeof(float),1,fp);
	datatype = FLOAT;
	fwrite(&datatype,sizeof(int32_t),1,fp);
    }
    /*
    ** write out the standard header to the correlation grid output
    ** file
	...
	*/
	...
	fval = 1.0;
	fwrite(&fval,sizeof(float),1,fp);
	fwrite(&(result->datatype),sizeof(int32_t),1,fp);
    }
    /*
...
int32_t	direction;
    ...
    */
    ninterpol = (int32_t)(POSITION_INTERVAL*1e4/result->binsize + 0.5);
    while(!feof(fp)){
	/*
	...
	*/
	...
	if(p2format){
	    ...
	    */
	    fread(&x1,sizeof(int16_t),1,fp);
	    fread(&y1,sizeof(int16_t),1,fp);
	    fread(&x2,sizeof(int16_t),1,fp);
	    fread(&y2,sizeof(int16_t),1,fp);
	    if(convert){
		ConvertData(&x1,sizeof(int16_t));
		ConvertData(&y1,sizeof(int16_t));
		ConvertData(&x2,sizeof(int16_t));
		ConvertData(&y2,sizeof(int16_t));
	    }
	} else {
	    /*
		...
		*/
		...
		if(convert){
		    ConvertData(&direction,sizeof(int32_t));
		}
	    }
	}
	/*
	...
	*/
	if(bin == pbin){
	    ...
	    */
		...
		speed = sqrt((double)(((int32_t)x1-px1)*((int32_t)x1-px1) + 
			((int32_t)y1-py1)*((int32_t)y1 - py1)))/
...
int32_t ComputeBayesianDistributions(result)
Result	*result;
{
int32_t	i;
int32_t	j;
