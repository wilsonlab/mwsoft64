...
int32_t CheckForOverlap(result,bin,cluster1,cluster2)
Result	*result;
int32_t	bin;
int32_t	cluster1;
int32_t	cluster2;
...
int32_t	nnonoverlap;
    ...
    */
    ...
    nbins = 2*(int32_t)(0.5 + (maxinterval/result->binsize)) + 1;
	...
	*/
	...
	for(j=start;j<nspikes2;j++){
	    ...
	    */
	    ...
	    split = 0;
	    if(interval >= 0){
		if(fbin - (int32_t)fbin == (0.5 - result->zerosqueeze) && 
		(int32_t)fbin != 0){
		    /* split it */
		    split = -1;
		}
		bin = zbin + (int32_t)(fbin + 0.5 + result->zerosqueeze);
	    } else {
		if(fbin - (int32_t)fbin == (-0.5 + result->zerosqueeze) &&
		(int32_t)fbin != 0){
		    /* split it */
...
uint32_t	*sarray1,*sarray2;
	...
	** these periods
	*/
	binarray1 = (int32_t *)calloc(result->ntimebins,sizeof(int32_t));
	binarray2 = (int32_t *)calloc(result->ntimebins,sizeof(int32_t));
	/*
...
int32_t FindPeak(result,corr_result,corrarray,expected_corr,sd_corr,bestphase,bestcorr,bestindex,bestcv)
Result	*result;
CorrResult	*corr_result;
float		*corrarray;
...
int32_t corrcompare(i1,i2)
CorrList	*i1,*i2;
{
    return((int32_t)(i1->corrsum - i2->corrsum > 0));
}
