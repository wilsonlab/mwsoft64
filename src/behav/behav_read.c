...
int32_t ReadAngularRangeMap(result)
Result	*result;
{
int32_t	headersize;
char	**header;
...
int32_t	convert;
    ...
    */
	...
	plist->npositions = (fsize - plist->headersize)/
	    (sizeof(uint32_t) + 4*sizeof(int16_t));
    } else {
	plist->npositions = (fsize - plist->headersize)/
	    (sizeof(uint32_t) + 4*sizeof(unsigned char));
    }

#ifdef OLD
    while(!feof(plist->fp)){		/* scan the entire file */
	if(fread(&p.timestamp,sizeof(uint32_t),1,plist->fp) != 1){
	    break;
	}
	...
	*/
	if(fread(&timestamp,sizeof(uint32_t),1,plist->fp) != 1){
	    break;
	}
	if(convert){
	...
	*/
	...
	if(p2format){
	    ...
	    */
	    ...
	    sizeof(int16_t),4,plist->fp) != 4){
		break;
	    }
	    if(convert){
		ConvertData(&plist->positionarray[count].xy[0],sizeof(int16_t));
		ConvertData(&plist->positionarray[count].xy[1],sizeof(int16_t));
		ConvertData(&plist->positionarray[count].xy[2],sizeof(int16_t));
		ConvertData(&plist->positionarray[count].xy[3],sizeof(int16_t));
	    }
	} else {
	    /*
	    ** read in the front and back diode positions
...
char	*tstr;
    ...
    */
    ...
    spike->nspikes = (fsize - spike->headersize)/(sizeof(uint32_t));
#ifdef OLD
    while(!feof(spike->fp)){		/* scan the entire file */
	if(fread(&timestamp,sizeof(uint32_t),1,spike->fp) != 1){
	    break;
	}
	spike->nspikes++;
    }
...
SpikeList *spike;
{
    ...
    */
    fseek(spike->fp,spike->headersize + startspike*sizeof(uint32_t),0L);
    if(fread(&starttime,sizeof(uint32_t),1,spike->fp) != 1){
    ...
    */
    fseek(spike->fp,spike->headersize + endspike*sizeof(uint32_t),0L);
    if(fread(&endtime,sizeof(uint32_t),1,spike->fp) != 1){
	fprintf(stderr,"ERROR: unable to find spike number %d\n",endspike);
    }
    if(spike->convert){
