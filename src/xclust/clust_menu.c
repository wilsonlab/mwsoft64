...
DataSource *GetCurrentSource()
{
  ...
  //   return(tstring);
  // }
void RunFindspike(char *args, char *out, int buflen)
{
  pid_t waitstatus;
  char command[200];
  char tstring[40];
  ...
  if (buflen<=40){
    didError = true;
    fprintf(stderr, "The provided buffer is too short: %d", buflen);
  }
  sprintf(command,"findspike %s ", args);
  ...
  if(tstring == NULL || WEXITSTATUS(waitstatus)){  
  /* empty answer from findspike or return value from findspike */
    switch (WEXITSTATUS(waitstatus)){
      case 0: /* Null tstring & return value = 0 (success) */
        fprintf(stderr,"ERROR: findspike returned no result\n");
        break;
  ...
  if (didError){
    ...
    fprintf(stderr, "An Error occured executing RunFindspike!\n");
    sprintf(out, "-1");
  
  }
  // There wasn't an error (non-null answer)
  else{
...
int32_t LookupSpikeIndex(DataSource *source, char *time)
{
  /*DataSource *source;*/
  char args[100];
  char result[100];
...
int32_t LookupSpikeTime(DataSource *source, char *index){
...
EndStandardHeader(fp);
  ...
  /*     } */
    ...
    sprintf(str_start, "%"PRId32, LookupSpikeIndex( NULL, start ) );
    sprintf(str_end, "%"PRId32, LookupSpikeIndex( NULL, end ) );
...
char *itemname;
{
  ...
  if (val!=NULL && val[0]!='\0') {
    ...
    sprintf(tmpstr, "%"PRId32, (int32_t) ( lrint( ParseTimestamp( val ) / tsperpoint ) ) );
    PutItemValue(itemname, tmpstr);
    RefreshItem(itemname);
  }
}
...
char *itemname;
{
  ...
  if (val!=NULL && val[0]!='\0') {
    ...
    int tempval = LookupSpikeIndex(NULL, val);
    
    char tempstr[50];
    sprintf(tempstr, "%"PRId32, tempval );
    //PutItemValue(itemname, LookupSpikeIndex(NULL, val));
    PutItemValue(itemname, tempval);
    RefreshItem(itemname);
  }
  ...
  /* Graph	*graph; */
    ...
    /* are start and end given in time or points? */
      ...
      /* convert to points first */
        ...
        pointend = LookupSpikeIndex(NULL, end);

        if (pointstart >= 0 && pointend > pointstart){
          LoadPointsAndUpdateStatus(pointstart, pointend, "");
          return;
        }
