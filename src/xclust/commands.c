...
{
	int i = 0;
	int32_t count = 1; // add 1 for the input argument?
	bool onChar = true;
...
char *line;
{
	uint32_t ARGC = CommandArgumentCount(line);
	Plot	*p;
...
if(Match("/randomizeprojectionbyname")){
	...
	if (ARGC < 2){
		fprintf(stderr,
			"ERROR: /randomizeprojectionbyname takes at least 2 args but got %"PRId32" (/randomizepojectionbyname <projection name> ... 0/1 ) : %s\n",
			ARGC,line);
		return;
	}
...
if(Match("/randomizeprojection")){
	...
	if (ARGC < 2){
		fprintf(stderr,
			"ERROR: /randomizeprojection takes at least 2 args but got %"PRId32" (/randomizepojection <projection index> ... 0/1 ) : %s\n",
			ARGC,line);
		return;
	}
