...
char	*fieldname;
	...
	if(strcmp(argv[nxtarg],"-usage") == 0){
	    ...
	    argv[0]);
	    fprintf(stderr,"\t[-ulong][-char][-int16_t][-int32_t][-float][-double][\n");
	    fprintf(stderr,"\t[-output ulong char int16_t int32_t float double]\n");
	    exit(0);
	} else
...
/* passed in from makefile when building an RPM*/
	...
	if(strcmp(argv[nxtarg],"-int16_t") == 0){
	    format = SHORT;
	} else
	...
	if(strcmp(argv[nxtarg],"-output") == 0){
	    ...
	    if(strcmp(outputformatstr,"int16_t") == 0){
		outputformat = SHORT;
	    } else
	    if(strcmp(outputformatstr,"int32_t") == 0){
		outputformat = INT;
	    } else
    ...
    */
	...
	break;
    case SHORT:
    	fprintf(bfp,"%% Fields: %s,%d,%d,%d\n",fieldname,SHORT,sizeof(int16_t),1);
	break;
    case INT:
    	fprintf(bfp,"%% Fields: %s,%d,%d,%d\n",fieldname,INT,sizeof(int32_t),1);
	break;
    case FLOAT:
	...
	break;
    case ULONG:
    	fprintf(bfp,"%% Fields: %s,%d,%d,%d\n",fieldname,ULONG,sizeof(uint32_t),1);
	break;
    }
	...
	*/
	...
	case ULONG:
		...
		case SHORT:
		    sval = ulval*scale;
		    fwrite(&sval,sizeof(int16_t),1,bfp);
		    break;
		case INT:
		    ival = ulval*scale;
		    fwrite(&ival,sizeof(int32_t),1,bfp);
		    break;
		case FLOAT:
		    fval = ulval*scale;
		...
		case ULONG:
		    ulval = ulval*scale;
		    fwrite(&ulval,sizeof(uint32_t),1,bfp);
		    break;
		}
	    }
	    break;
