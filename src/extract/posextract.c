...
int32_t IsInPolygon(result,wx,wy,fcoord,ncoords)
Result	*result;
float	wx,wy;
FCoord	*fcoord;
int32_t	ncoords;
...
int32_t GetPosRecord(Result *result,PositionRecord *posrec)
{
int32_t	size;
int32_t	i;
...
/*     } */
    ...
    */
    if(fread(&posrec->timestamp,sizeof(uint32_t),1,result->fpin) != 1){
	if(feof(result->fpin)) return(0);
    ...
    if(convert){
	ConvertData(&posrec->timestamp,sizeof(uint32_t));
    }
    if(result->toffset > 0){
    ...
    ** read in all of the coords
	...
	(int32_t)posrec->nitems,(int32_t)posrec->frame,
    ...
    */
	...
	if(fread(&posrec->coord[i].xcoord,sizeof(int16_t),1,result->fpin) != 1){
	    if(feof(result->fpin)) return(0);
	...
	if(convert){
	    ConvertData(&posrec->coord[i].xcoord,sizeof(int16_t));
	}
	if(dump && singlecolumn)
...
void ComparePixels(PositionRecord *posrec1, PositionRecord *posrec2,int32_t
match_direction)
{
int16_t	xcoord1;
...
int32_t PixelDiff(PositionRecord *posrec1, PositionRecord *posrec2)
{
int16_t	xcoord1;
unsigned char ycoord1;
int16_t	xcoord2;
...
void Smooth(Result *result, int16_t **grid,int16_t xcoord,unsigned char ycoord)
{
int32_t x,y;
int32_t i,j;
...
int32_t EvaluateQueuedDifference(Result *result, PositionRecord *posrec,
    int32_t recnum,float *distance, int32_t *pixeldiff)
{
int16_t	xcoord1;
unsigned char ycoord1;
int16_t	xcoord2;
...
int16_t	*gridptr2;
    ...
    */
    for(i=0;i<XSIZE;i++){
	bzero(grid[i],YSIZE*sizeof(int16_t));
	bzero(grid2[i],YSIZE*sizeof(int16_t));
    }
    /*
	...
	*/
	...
	||  ((result->filter_points == 2) && 
	    ...
	    ** check to see whether the coord is too far
	    if((count ==  0) || 
	    (abs((int32_t)(posrec->coord[i].xcoord - xcoord/count))<MAXDIODESPREAD
	    && abs((int32_t)(posrec->coord[i].ycoord - ycoord/count))<MAXDIODESPREAD)
	    ){
	    */
...
uint32_t mininterval;
    ...
    result.fcoord = NULL;
    jump = 0;
    grid = (int16_t **)malloc(sizeof(int16_t *)*XSIZE);
    grid2 = (int16_t **)malloc(sizeof(int16_t *)*XSIZE);
    for(i=0;i<XSIZE;i++){
	grid[i] = (int16_t *)malloc(sizeof(int16_t)*YSIZE);
	grid2[i] = (int16_t *)malloc(sizeof(int16_t)*YSIZE);
    }
...
/* passed in from makefile when building an RPM*/
    ...
    */
	...
	"%% Fields:\t%s,%d,%d,1\t%s,%d,%d,1\n",
	    "x",SHORT,sizeof(int16_t),
	    "y",SHORT,sizeof(int16_t));
    } else {
	...
	"%% Fields:\t%s,%d,%d,1\t%s,%d,%d,1\t%s,%d,%d,1\t%s,%d,%d,1\t%s,%d,%d,1\n",
	    "timestamp",ULONG,sizeof(uint32_t),
	    "xfront",SHORT,sizeof(int16_t),
	    "yfront",SHORT,sizeof(int16_t),
	    "xback",SHORT,sizeof(int16_t),
	    "yback",SHORT,sizeof(int16_t));
	fprintf(result.fpout,"%%\n%% Pos file header:\n");
	    ...
	    */
        ...
        if(result.ignorefb){
            if(got_rear || (frontfill && got_front && count > 1)){
                ...
                */
                fwrite(&rear_time,sizeof(uint32_t),1,result.fpout);
		...
		if(result.fpstatout){
		    ...
		    */
            ...
            if(got_front || (backfill && got_rear && count > 1)){
                ...
                ** write out the position
                */
                fwrite(&front_time,sizeof(uint32_t),1,result.fpout);
