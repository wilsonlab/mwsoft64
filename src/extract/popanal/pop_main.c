...
int32_t	tmpcount;
	...
	*/
	if((result.spikearray = (uint32_t **)malloc(result.nclusters*
	sizeof(uint32_t *)))
	== NULL){
	    fprintf(stderr,
    ...
    */
    if((result.spikecount = (int32_t *)calloc(result.nclusters,sizeof(int32_t)))
    == NULL){
	fprintf(stderr,
	...
	*/
	...
	if((result.reconspikecount = (int32_t *)calloc(result.nreconclusters,sizeof(int32_t)))
	== NULL){
	    fprintf(stderr,
	    "ERROR: out of memory. Unable to allocate spike count array\n");
	    exit(-1);
