...
typedef struct result_type {
    ...
    int32_t		norm1;
    int32_t		norm2;
    int32_t		suppresszero;
    int32_t		ignoremultiple;
    int32_t		normcorr;
    uint32_t starttime;
    uint32_t endtime;
    int32_t		eegthresh;
...
int32_t	maxbins = MAXBINS; 

int32_t InRange(result,timestamp,edgeoffset)
Result	*result;
uint32_t	timestamp;
uint32_t	edgeoffset;
{
...
uint32_t *LoadSpikes(fp,nspikes,size)
FILE	*fp;
int32_t	*nspikes;
int32_t 	*size;
{
...
int32_t	convert;
    ...
    */
    fseek(fp,0L,2);
    *nspikes = (ftell(fp) - *size)/sizeof(uint32_t);
    sarray = (uint32_t *)malloc((*nspikes)*sizeof(uint32_t));
    /*
...
uint32_t 	toffset;
    ...
    */
    ...
    result->nbins = 2*(int32_t)(0.5 + (maxinterval/result->binsize)) + 1;
    if(result->nbins > maxbins){
	result->nbins = maxbins;
    }
    /*
	...
	*/
	...
	for(j=start;j<result->nspikes2;j++){
	    ...
	    */
		...
		bin = zbin + (int32_t)(fbin + 0.5 + split);
	    } else {
		if((fbin - (int32_t)fbin) == -0.5){
		    /*
		    ** randomly assign it to one side or the other
...
int32_t	bin;
int32_t	firstflag;
int32_t	min_isi = INT_MIN;
int32_t	max_isi = INT_MAX;
int32_t	i;
 ...
 */
 ...
 if(result->nbins <= 0){
    ...
    */
    result->isiarray = (uint32_t *)calloc(result->nbins,
	sizeof(uint32_t));
    /*
...
*/
int32_t TestSpikes(result,spikearray,nspikes)
Result	*result;
uint32_t *spikearray;
int32_t	nspikes;
{
...
int32_t	count;
	...
	result->spikearray = LoadSpikes(result->fpin[0],&result->nspikes,
	    &result->headersize);
	qsort(result->spikearray,result->nspikes,sizeof(uint32_t),
	    intcompare);
	/*
	...
	*/
	...
	result->spikearray2 = LoadSpikes(result->fpin[1],&result->nspikes2,
	    &result->headersize2);
	qsort(result->spikearray2,result->nspikes2,sizeof(uint32_t),
	    intcompare);
	/*
	** test the spike array for proper form
	*/
...
,
	...
	*/
	if(fread(&startstamp,sizeof(uint32_t),1,result->fpcr) != 1) break;
	if(fread(&nsamples,sizeof(int16_t),1,result->fpcr) != 1) break;
	if(fread(&freq,sizeof(uint32_t),1,result->fpcr) != 1) break;
	if(fread(crdata,sizeof(int16_t),nsamples,result->fpcr) != nsamples) break;
	if(convert){
	...
	*/
	for(i=0;i<nsamples;i++){
	    ...
	    */
	    if(!InRange(result,(uint32_t)time1,0)) continue;
	    ca = result->corrarray;
	    firsttime = 1;
...
uint32_t windowmax;
	...
	if(strcmp(argv[nxtarg],"-usage") == 0){
	    ...
	    argv[0]);
	    fprintf(stderr,"\t[-tmax msec][-v][-rate][-int32_t][-sparse][-edge]\n");
...
/* passed in from makefile when building an RPM*/
	...
	if(strcmp(argv[nxtarg],"-int32_t") == 0){
