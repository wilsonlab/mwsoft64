...
float	fval;
	...
	ival = 359;
	fwrite(&ival,sizeof(int32_t),1,result->fp);
	ival = 0;
	fwrite(&ival,sizeof(int32_t),1,result->fp);
	if(result->binsize > 0){
	...
	if(result->fpvar){
	    ival = result->xsize-1;
	    fwrite(&ival,sizeof(int32_t),1,result->fpvar);
	    ival = result->ysize-1;
	    fwrite(&ival,sizeof(int32_t),1,result->fpvar);
	    fval = 1;
...
float tmp, num;
	...
	TimestampToString((int32_t)(result->totaltime*result->pinterval*1e4)),
...
int16_t	sval;
    ...
    if(fwrite(&timestamp,sizeof(uint32_t),1,result->fpfix) != 1){
	fprintf(stderr,"ERROR: writing to fix file\n");
	exit(-1);
    }
    sval = x1;
    if(fwrite(&sval,sizeof(int16_t),1,result->fpfix) != 1){
	fprintf(stderr,"ERROR: writing to fix file\n");
	exit(-1);
    }
    sval = y1;
...
uint32_t	lval;
    ...
    lval = timestamp;
    if(fwrite(&lval,sizeof(uint32_t),1,result->fprate) != 1){
    ...
    sval = dir;
    if(fwrite(&sval,sizeof(int16_t),1,result->fprate) != 1){
	fprintf(stderr,"ERROR: writing to rate file\n");
	exit(-1);
    }
    fval = rate;
